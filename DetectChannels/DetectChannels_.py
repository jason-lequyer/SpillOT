...
try:
    ...
    groups_by_name  = [
        ...
        ["CD16", "CD15"],
        ["CD193", "Siglec-8"],
        [u"Fc\u03b5R1\u03b1", "CD63", "CD203c"],
        [u"Fc\u03b5R1\u03b1", "CD117", "Tryptase"],
        ["PanCK", "pan-Keratin"],
        ["DNA1", "DNA2"],
        ["DAPI", "DNA1", "DNA2"],
    ]
    ...
    from java.awt import (Dimension, Toolkit, BorderLayout, GraphicsEnvironment,
                          ...
                          Color, BasicStroke, RenderingHints, Cursor)
    ...
    def _u(x):
        ...
        try:
            ...
            pass
        try:
            return unicode(x, "utf-8") if isinstance(x, str) else unicode(x)
        except Exception:
            try:
                return unicode(str(x), "utf-8", "replace")
            except Exception:
                try:
                    return unicode(str(x), "latin-1", "replace")
                except Exception:
                    return u""
    ...
    def _dimension_for_screen_fraction(fraction=0.70, min_w=600, min_h=450):
        _x, _y, usable_w, usable_h = _usable_screen_bounds()
        target_w = int(round(float(usable_w) * float(fraction)))
        target_h = int(round(float(usable_h) * float(fraction)))
        w = max(300, min(int(usable_w), max(int(min_w), target_w)))
        h = max(300, min(int(usable_h), max(int(min_h), target_h)))
    ...
    def _paint_selection_ring(component, g, selected):
        """Draw the selection outline after child components paint, over the preview."""
        try:
    ...
    def _select_planes_to_debleed(names, imp, axis_used):
        ...
        dlg = JDialog(None, "DetectChannels: select %ss" % label, True)
        dlg.setLayout(BorderLayout())

        # Top message (header only; remove everything underneath)
        top = JPanel()
        top.setLayout(BoxLayout(top, BoxLayout.Y_AXIS))
        msg = (u"<html><b>Select which %ss to process</b></html>") % (_u(label),)
        top.add(JLabel(msg))
        ...
        def on_ok(_):
            ...
            if not sel:
                JOptionPane.showMessageDialog(
                    dlg,
                    u"Select at least one %s to process." % _u(label),
                    "Nothing selected",
                    JOptionPane.WARNING_MESSAGE
                )
                return
    ...
    def _csv_cell_means_keep(v):
        # Legacy DetectChannels matrix semantics:
        #   1 or -1  => keep / allow this comparison
        #   0, blank, NaN, or any other value => exclude / veto this comparison
        try:
            txt = _u(v).replace(u"\ufeff", u"").strip()
            if txt == u"":
                return False
            fv = float(txt)
    ...
    def _load_groups_from_saved_csv(csv_path, names, return_zero_pairs=False):
        ...
        try:
            ...
            for rline in lines[1:]:
                ...
                vals = cells[1:]
                for j, col_idx in enumerate(col_to_idx):
                    v = vals[j] if j < len(vals) else ""
                    # Keep only explicit +1/-1 cells. Everything else is an exclusion.
                    if row_idx != col_idx and not _csv_cell_means_keep(v):
    ...
    # Dock-safe sizing: default to about 70% of the usable screen, centered by JOptionPane.
    try:
        panel.setPreferredSize(_dimension_for_screen_fraction(0.70, 700, 500))
    except Exception:
        ge = GraphicsEnvironment.getLocalGraphicsEnvironment()
        gc = ge.getDefaultScreenDevice().getDefaultConfiguration()
        scr = Toolkit.getDefaultToolkit().getScreenSize()
    ...
    # ---------------------------------------------------------------------
    # CSV semantics for legacy DetectChannels matrices:
    #   1 or -1 => keep / allow comparison
    #   0 or blank => exclude / veto comparison
    # New CSVs written by this launcher use 1 for allowed and 0 for excluded.
    # Preserve old excluded pairs among totally-unselected planes if CSV existed.
    # ---------------------------------------------------------------------
    ...
    def _find_runner_script(keep_brightest):
        runner_name = "keep_the_brightest.py" if keep_brightest else "signal_based.py"
        cand = []
        ...
        if plugins_dir:
            ...
            cand.append(os.path.join(plugins_dir, "DetectChannels", runner_name))
            cand.append(os.path.join(plugins_dir, "DetectChannels", "bin", runner_name))
    ...
    if not runner_py:
        if keep_brightest:
            abort("Could not find 'keep_the_brightest.py'. Put it beside DetectChannels_.py, or in:\n"
                  + os.path.join(IJ.getDir('plugins'), "DetectChannels"))
        else:
            abort("Could not find 'signal_based.py'. Put it beside DetectChannels_.py, or in:\n"
                  + os.path.join(IJ.getDir('plugins'), "DetectChannels"))

...
5. Enter seettings. Patch size must be an even integer greater than or equal to 4. The default is `16` and this usally works well. Smaller patch sizes are usually more aggressive and faster. Larger patch sizes are for slower and more gentle correction. The ignore overexposed checkbox is rarely needed, but can be useful when saturated pixels are causing artefacts.
